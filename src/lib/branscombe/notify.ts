/**
 * Branscombe notification dispatcher. Mirrors lib/seafields/notify.ts.
 * Recipients come from branscombe_notify_recipients (managed via
 * /admin/branscombe-pipeline). Hardcoded fallback (Dennis + Uwe) so a
 * misconfigured table never strands the path.
 */
...
export function renderBrandedEmail({
  ...
  const { primary, accent, ink, muted, surface } = BRANSCOMBE_BRAND;
  const safeRows = rows
    .map(
      (r) => `
      <tr>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${surface};padding:32px 16px">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#FFFFFF;border:1px solid #E2E8F0;border-radius:6px;overflow:hidden">
        <tr>
          <td style="background:${primary};color:#FFFFFF;padding:18px 24px;font-family:Arial,sans-serif">
            <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#94A3B8">Branscombe Estate · Admin</div>
      ...
      <div style="margin-top:14px;color:${muted};font-size:11px;font-family:Arial,sans-serif">
        You receive these because you are on the Branscombe notification list. Manage at
        <a href="https://f2k-projects.vercel.app/admin/branscombe-pipeline" style="color:${muted}">/admin/branscombe-pipeline</a>.
      </div>
    </td></tr>
  </table>
</body></html>`;
