...
export async function GET() {
  ...
  if (!admin || !hasPermission(admin.role, "manage_hemp_homes_notifications")) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const supabase = createSupabaseService();
  const { data, error } = await (
    supabase.from("hemp_homes_notify_recipients") as any
  )
...
export async function POST(req: Request) {
  ...
  await auditLog(
    admin.id,
    admin.email,
    "hemp_homes_notify_recipient_added",
    "hemp_homes_notify_recipients",
    email,
    { name },
  );
...
export async function DELETE(req: Request) {
  ...
  await auditLog(
    admin.id,
    admin.email,
    "hemp_homes_notify_recipient_removed",
    "hemp_homes_notify_recipients",
    email,
    {},
  );
